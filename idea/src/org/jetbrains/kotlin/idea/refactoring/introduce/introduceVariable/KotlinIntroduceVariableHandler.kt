...
object KotlinIntroduceVariableHandler : KotlinIntroduceHandlerBase() {
    ...
    private fun KtExpression.getCandidateContainers(
        ...
        fun isResolvableNextTo(neighbour: KtFunctionLiteralExpression): Boolean {
        ...
        if (!firstContainer.isLambdaBlock()) return listOf(firstContainer to firstOccurrenceContainer)

        val lambdasAndContainers = ArrayList<Pair<KtFunctionLiteralExpression, KtElement>>().apply {
            var container = firstContainer
            do {
                val lambda = container.getStrictParentOfType<KtFunctionLiteralExpression>()!!
                if (!isResolvableNextTo(lambda)) return@apply
                container = lambda.getContainer() ?: return@apply
                add(lambda to container)
            } while (container.isLambdaBlock())
        }
